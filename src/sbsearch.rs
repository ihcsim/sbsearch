...
fn search_file(
    ...
    searcher.search_path(
        ...
        UTF8(|_lnum, line| {
            ...
            let level;
            if let Ok(r) = find_log_level(line) {
                level = r;
            } else {
                level = "UNKNOWN";
            }
...
fn find_log_level(line: &str) -> Result<&str, Box<dyn Error>> {
    ...
    if let Ok(opt) = matcher_log_level1.find(line.as_bytes())
        && let Some(m) = opt
    {
        Ok(line[m.start()..m.end()].split('=').nth(1).unwrap())
    } else if let Ok(opt) = matcher_log_level2.find(line.as_bytes())
        && let Some(m) = opt
    {
        Ok(line[m.start()..m.end()]
            ...
            .unwrap()
            .trim_matches('"'))
    } else if let Ok(opt) = matcher_log_level3.find(line.as_bytes())
        && opt.is_some()
    {
        Ok("error")
    } else if let Ok(opt) = matcher_log_level4.find(line.as_bytes())
        && opt.is_some()
    {
        Ok("error")
    } else {
...
mod tests {
    ...
    fn test_find_log_level_pattern1() {
        ...
        let expected = "info";
        let actual = find_log_level(line).unwrap();
